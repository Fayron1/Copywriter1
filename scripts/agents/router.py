...
class ArticleRouter:
    ...
    def route(topic: str, description: str, size: str, keywords: List[str], custom_chars: int = 0) -> Dict[str, Any]:
        ...
        # Compact Mode: статьи до 5000 символов — упрощённая структура/SEO.
        # НЕ активируется для чек-листов (у них своя адаптивность 5/7/10 пунктов).
        compact_mode = (target_chars < 5000) and (article_type != "checklist")
        ...
        if keywords:
            ...
            if compact_mode:
                # COMPACT: ослабленные требования, чтобы не раздувать короткую статью.
                # 1 H2 с ключом вместо 2; FAQ 2 вопроса вместо 3-5; без отдельного блока ошибок.
                seo_instructions["engineer_instruction"] = (
                    f"\n\n=== ЕДИНЫЙ SEO-СЛОЙ (КОМПАКТНЫЙ РЕЖИМ) ===\n"
                    f"- Включи в план один H1 с основной ключевой фразой: '{primary_key}'.\n"
                    f"- Включи хотя бы ОДНО ключевое слово из списка {keywords_str} в заголовок раздела H2.\n"
                    f"- НЕ проектируй отдельный блок «Типичные ошибки» — короткой статье он вреден.\n"
                    f"- В конце — блок 'FAQ' из 2 коротких вопросов с ответами (НЕ 3-5)."
                )
                seo_instructions["heart_instruction"] = (
                    f"\n\n=== ЕДИНЫЙ SEO-СЛОЙ (КОМПАКТНЫЙ РЕЖИМ) ===\n"
                    f"- Используй ключевую фразу '{primary_key}' в заголовке H1.\n"
                    f"- Включи ключевые слова {keywords_str} естественно в текст: хотя бы в одном подзаголовке H2.\n"
                    f"- Пиши максимально плотно, без длинных вступлений-крючков (статья до 5000 символов).\n"
                    f"- FAQ в конце — 2 вопроса, кратко."
                )
                seo_instructions["sheriff_instruction"] = (
                    f"\n\n=== ЕДИНЫЙ SEO-СЛОЙ (Проверка, компакт) ===\n"
                    f"1. Проверь H1 с ключевой фразой '{primary_key}'.\n"
                    f"2. Проверь вхождение хотя бы одного ключевого слова {keywords_str} в одном H2.\n"
                    f"3. FAQ — 2 вопроса (не более)."
                )
            else:
                seo_instructions["engineer_instruction"] = (
                    f"\n\n=== ЕДИНЫЙ SEO-СЛОЙ ===\n"
