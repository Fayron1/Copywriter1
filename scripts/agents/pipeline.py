...
class Pipeline:
    ...
    def _generate_clean_heart_text(self, user_msg: str, max_retries: int = 2, target_chars: int = 0, state: PipelineState = None) -> str:
        ...
        for attempt in range(max_retries):
            result = self._call_agent("heart", current_msg, parse_json=False, target_chars=target_chars, state=state)
    ...
    def _heart_sectional(self, state, style_block, rag_block, target_chars):
        ...
        for i, section in enumerate(sections, 1):
            ...
            result = self._generate_clean_heart_text(section_msg, target_chars=chars_per_section, state=state)
            parts.append(result)
        ...
        if state.style_id == "checklist":
            ...
            conclusion_text = self._generate_clean_heart_text(conclusion_msg, target_chars=500, state=state)
            full_article += "\n\n" + conclusion_text
    ...
    def _call_agent(
        ...
        if state is not None:
            ...
            if provider == "kie":
                # Гибрид: kie.ai через помодельный путь (см. _get_kie_client).
