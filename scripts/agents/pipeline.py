...
class Pipeline:
    ...
    def _get_kie_client(self, model_name: str):
        ...
        if model_name not in self._kie_model_clients:
            ...
            self._kie_model_clients[model_name] = OpenAI(
                api_key=self._kie_api_key, base_url=base, timeout=120.0
            )
    ...
    def preflight(self, check_image: bool = True, provider: str = "deepseek",
        ...
        if prov == "openai":
            ...
            kie_models = {MODELS["external_reviewer"], MODELS["kie_text"], "gpt-5.5", "gemini-3.1-pro-openai", "gpt-5-5-openai-resp"}
            ids = ids - kie_models
    ...
    def run(
        ...
        if quality_mode is None:
            quality_mode = os.getenv("QUALITY_MODE", "true").lower() in ("1", "true", "yes", "on")
        ...
        try:
            ...
            if state.quality_mode:
                ...
                # Объединённый review loop (Sheriff + условный Mirror → единая ревизия)
                # Адаптивно: маленьким статьям (≤8000) хватает 1 прохода, лонгридам — 2.
                _review_target = state.custom_chars or state.max_chars or 0
                max_review_iters = 1 if (0 < _review_target <= 8000) else 2
                for i in range(max_review_iters):  # макс. итераций объединённого review
                    # Sheriff проверяет
                    ...
                    if sheriff_score < 75:
                        ...
                        logger.info(f"   ⏭️ Mirror пропущен (Turing Score: {sheriff_score} ≥ 75)")
    ...
    def _step_plan_critique(self, state: PipelineState):
        ...
        if isinstance(response, dict) and not response.get("ok", True):
            ...
            if issues or missing or redundant:
                ...
                # Загружаем базовый промпт для повторного вызова
                original_user_msg = (
                    f"Тема: {state.topic}\n"
                    f"Направление: {state.direction}\n\n"
                    f"ФАКТЫ:\n{self._compact_json(state.facts, 5000)}\n"
                ...
                retry_msg = (
                    ...
                    f"Пожалуйста, перепиши его, исправив следующие замечания:\n"
                )
                for issue in issues:
    ...
    def _step_heart(self, state: PipelineState):
        ...
        if state.quality_mode:
            state.draft = self._step_heart_best_of(state, style_block, rag_block, heart_target)
        else:
    ...
    def _step_heart_best_of(self, state: PipelineState, style_block: str, rag_block: str, heart_target: int) -> str:
        ...
        if self._kie_api_key:
            drafts_configs = [
                ...
                {"provider": "kie", "model": "gemini-3.1-pro-openai", "temperature": 0.85, "label": "Gemini 3.1 Pro (KIE)"},
                {"provider": "kie", "model": "gpt-5-5-openai-resp", "temperature": 0.7, "label": "GPT 5.5 (KIE)"}
            ]
        else:
            drafts_configs = [
        ...
        for idx, cfg in enumerate(drafts_configs, 1):
            ...
            try:
                ...
                # Оцениваем черновик ревизором
                eval_res = self._evaluate_draft_score(state, draft_text)
    ...
    def _step_quality_edit_loop(self, state: PipelineState):
        ...
        # Получим первоначальную оценку черновика
        logger.info("   🔍 Оценка исходного черновика ревизором...")
        initial_eval = self._evaluate_draft_score(state, state.draft)
        best_score = initial_eval["score"]
        logger.info(f"   📈 Исходный балл: {best_score}/100. Вердикт: {initial_eval['critique']}")
        
        if best_score >= 90:
            logger.info("   ✅ Исходный черновик уже идеального качества (>= 90). Правки не требуются.")
            return
        ...
        for iteration in range(MAX_QUALITY_ITERS):
            ...
            if response.get("approved") == True or score >= 90 or not edits:
                logger.info(f"      ✅ Ревизор одобрил статью (балл {score} >= 90 или approved=True).")
                break
    ...
    def _generate_clean_heart_text(self, user_msg: str, max_retries: int = 2, target_chars: int = 0, state: PipelineState = None, override_model=None, override_provider=None, override_temperature=None) -> str:
        ...
        for attempt in range(max_retries):
            ...
            if len(found_words) >= 3 and attempt < 1:
                ...
                current_msg = user_msg + f"\n\nSYSTEM ALERT: В тексте обнаружено много штампов: {found_words}. Перепиши текст, используя точную, деловую B2B-лексику без этих фраз."
                continue
                
            # 2. Проверяем оверобъем: максимум 1 повторный запрос на сжатие (attempt < 1)
            if state is not None and state.max_chars > 0 and len(text) > state.max_chars * 1.15 and attempt < 1:
                logger.warning(f"   ⚠️ [Volume Check] Текст слишком длинный ({len(text)} симв.). Просим ужать (попытка {attempt+1}/{max_retries})")
                current_msg = user_msg + (
                    f"\n\nSYSTEM ALERT: Твой предыдущий ответ содержал {len(text)} символов, что значительно превышает лимит {state.max_chars} символов. "
                    f"Напиши текст заново, сократив объем каждого раздела и убрав воду, но сохранив все заголовки."
                )
                continue
    ...
    def _heart_sectional(self, state, style_block, rag_block, target_chars, override_model=None, override_provider=None, override_temperature=None):
        ...
        for i, section in enumerate(sections, 1):
            ...
            if custom_section_chars and isinstance(custom_section_chars, (int, float)):
                chars_per_section = int(custom_section_chars)
            else:
                if state.style_id == "checklist":
    ...
    def _step_booster(self, state: PipelineState):
        ...
        min_expected_len = int(len(state.draft) * 0.8) if state.draft else 0
        if optimized_text and len(optimized_text) >= min_expected_len:
            state.final_article = optimized_text
            logger.info("   ✅ SEO-оптимизированный текст успешно интегрирован.")
        else:
            state.final_article = state.draft or ""
            logger.warning(
                f"   ⚠️ Оптимизированный текст от Booster слишком короткий ({len(optimized_text or '')} vs черновик {len(state.draft or '')}). "
                f"Откат на исходный черновик."
            )
        ...
        if not state.final_meta.get("subtitle"):
            try:
                ...
                subtitle_text = self._call_agent("booster", sub_msg, parse_json=False, state=state)
                state.final_meta["subtitle"] = subtitle_text.strip().strip('"«»')
        ...
        h1_match = re.search(r"^(#\s+.*?)$", state.final_article, re.MULTILINE)
        if h1_match:
            h1_line = h1_match.group(1)
            state.final_article = state.final_article.replace(h1_line, f"# {best_title}", 1)
    ...
    def _apply_smart_hard_cut(self, state: PipelineState):
        ...
        # 2. Остальные форматы: ОДНО экспресс-сжатие через flash.
        #    (Раньше тут дополнительно гонялся ещё и condense — двойной прогон всей статьи.)
        logger.info(f"   ⚠️ [Hard-Cut] Статья длинная ({len(source_text)} символов). Лимит: {limit}. Экспресс-сжатие через flash-модель...")
        try:
            ...
            compress_msg = (
                f"ТЕКСТ СТАТЬИ:\n{source_text}\n\n"
                f"Этот текст превышает лимит. Пожалуйста, сожми статью строго до {target} символов. "
            ...
            compressed_text = self._call_agent("heart", compress_msg, parse_json=False, target_chars=target, state=state)
            
            get_agent("heart").model = old_model  # возвращаем модель
            
            if len(compressed_text) < len(source_text) and len(compressed_text) > target * 0.6:
                source_text = compressed_text
                logger.info(f"   ✅ [Hard-Cut] Статья успешно сжата через flash до {len(source_text)} символов.")
                if len(source_text) <= limit:
                    state.final_article = source_text
                    return
        except Exception as e:
            logger.warning(f"   ⚠️ [Hard-Cut] Экспресс-сжатие завершилось ошибкой: {e}")

        # 4. Третья линия защиты (аварийный жесткий срез) — только для остальных типов
        logger.warning(f"   ✂️ [Hard-Cut] Вынужден применить жесткую обрезку (цель: {target} символов)...")
