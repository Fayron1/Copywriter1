...
SYSTEM_PROMPT = (
    ...
    "6. ОТВЕТСТВЕННОСТЬ ЗА НОМЕРА СТАТЕЙ: если в факте указана статья закона (НК РФ,\n"
    "   ТК РФ, КоАП, ФЗ), отдельным поиском проверь, что статья с этим номером ДЕЙСТВИТЕЛЬНО\n"
    "   регулирует тему факта. Пример: ст. 185.1 ТК РФ — это диспансеризация/медосмотры,\n"
    "   а НЕ сверхурочные; если факт ссылается на 185.1 в контексте переработок — это\n"
    "   перепутанный номер, пометь verdict='conflict' и в new_value укажи правильную статью\n"
    "   (для сверхурочных это ст. 99/152 ТК РФ), если удалось её найти.\n\n"
