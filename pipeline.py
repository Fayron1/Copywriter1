...
class Pipeline:
    ...
    def _generate_clean_heart_text(self, user_msg: str, max_retries: int = 2, target_chars: int = 0, state: PipelineState = None, override_model=None, override_provider=None, override_temperature=None) -> str:
        ...
        for attempt in range(max_retries):
            ...
            if current_max_chars > 0 and len(text) > current_max_chars * 1.15 and attempt < 1:
                logger.warning(f"   ⚠️ [Volume Check] Текст слишком длинный ({len(text)} симв. vs лимит {current_max_chars}). Просим ужать (попытка {attempt+1}/{max_retries})")
                current_msg = user_msg + (
                    f"\n\nSYSTEM ALERT: Твой предыдущий ответ содержал {len(text)} символов, что значительно превышает лимит {current_max_chars} символов. "
                    f"Напиши текст заново, сократив объем и убрав воду."
                )
                continue
    ...
    def _step_statistical_humanize(self, state: PipelineState):
        ...
        if result["text"] and len(result["text"]) > len(text) * 0.6:
    ...
    def _step_booster(self, state: PipelineState):
        ...
        if not state.final_meta.get("subtitle"):
            try:
                ...
                cleaned_sub = re.sub(r'<seo_metadata>.*?</seo_metadata>', '', cleaned_sub, flags=re.DOTALL)
                cleaned_sub = re.sub(r'<optimized_article>.*?</optimized_article>', '', cleaned_sub, flags=re.DOTALL)
